...
export default async function BuscarPage({ searchParams }: Props) {
  ...
  let notes: Array<{ id: string; title: string; slug: string; excerpt: string | null; published_at: string | null }> = []

  if (q && q.trim().length >= 2) {
    const { data } = await supabase.from('notes').select('id,title,slug,excerpt,published_at')
      .eq('published', true).or(`title.ilike.%${q}%,body.ilike.%${q}%,excerpt.ilike.%${q}%`)
      .order('published_at', { ascending: false }).limit(20)
    notes = data ?? []
  } else if (tag) {
    const { data: t } = await supabase.from('tags').select('id').eq('slug', tag).maybeSingle()
    if (t) {
      const { data } = await supabase.from('note_tags').select('note:notes(id,title,slug,excerpt,published_at,published)').eq('tag_id', t.id)
      notes = (data ?? []).map((r: { note: unknown }) => r.note).filter(Boolean)
        .filter((n: { published: boolean }) => n.published).slice(0, 20) as typeof notes
    }
  }
    ...
    <SiteLayout>
      <div className="max-w-2xl mx-auto px-6 py-20">
        ...
        {hasSearch ? (
          notes.length > 0 ? (
            <>
              <p className="text-eyebrow mb-6">{notes.length} resultado{notes.length !== 1 ? 's' : ''}{q ? ` para "${q}"` : ''}</p>
              {notes.map(n => (
            ...
            <div className="py-16 text-center">
              ...
              <p className="text-sm text-stone-300">Prueba con otras palabras o explora las <Link href="/notas" className="underline hover:text-stone-500 transition-colors">notas</Link>.</p>
            </div>
          )
        ) : (
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {['miedo', 'perdón', 'paz', 'ansiedad', 'amor', 'culpa', 'soltar', 'presencia'].map(term => (
